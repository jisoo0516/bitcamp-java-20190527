...
public class MemberCsvDao extends AbstractCsvDataSerializer<Member, Integer> implements MemberDao {
  ...
  protected Member createObject(String[] values) {
    ...
    member.setPassword(Integer.parseInt(values[3]));
    member.setPhoneNum(values[4]);
    member.setPicture(values[5]);
  ...
  protected String createCSV(Member obj) {
    return String.format("%d,%s,%s,%s,%s,%s,%s", obj.getNo(), obj.getName(), obj.getEmail(),
        obj.getPassword(), obj.getPhoneNum(), obj.getPicture(), obj.getJoinDate());
