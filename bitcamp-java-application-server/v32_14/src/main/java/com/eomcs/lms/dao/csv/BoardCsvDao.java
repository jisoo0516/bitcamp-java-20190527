...
public class BoardCsvDao extends AbstractCsvDataSerializer<Board, Integer> implements BoardDao {
  ...
  protected Board createObject(String[] values) {
    // 파일에서 읽어들인 데이터의 CSV형식은 다음과 같다고 가정하자!
    // => 번호,내용,생성일,조회수
    //
    Board board = new Board();
    board.setNo(Integer.parseInt(values[0]));
    board.setContents(values[1]);
    board.setReportingDate(Date.valueOf(values[2]));
    board.setHits(Integer.parseInt(values[3]));
  ...
  protected String createCSV(Board obj) {
    ...
    return String.format("%d,%s,%s,%d", obj.getNo(), obj.getContents(), obj.getReportingDate(),
        obj.getHits());
  }
