...
public class App {
  ...
  private static void loadLessonData() {
    ...
    try {
      ...
      while (scan.hasNextLine()) {
        ...
        // 문자열을 콤마로 분리한다. 불리된 데이터는 배열에 담겨 리턴된다.
  ...
  private static void saveLessonData() {
    ...
    try {
      ...
      // JVM을 멈추지 말고 간단히 오류 안내 문구를 출력한 다음에
      // 계속 실행하게 하자! System.out.println("파일에 데이터를 출력하는 중에 오류 발생!" System.out.println();
  ...
  private static void loadMemberData() {
    ...
    try {
      ...
      while (scan.hasNextLine()) {
        ...
        member.setPicture(data[4]);
        member.setPhoneNum(Integer.parseInt(data[5]));
        member.setJoinDate(Date.valueOf(data[6]));
    ...
    try {
      ...
      for (Member member : memberList) {
        ...
        String str = String.format("%d,%s,%s,%s,%s,%s,%s\n", member.getNo(), member.getName()
            , member.getEmail(), member.getPassword(), member.getPicture(), member.getPhoneNum(),
            member.getJoinDate());
  ...
  private static void loadBoardData() {
    ...
    try {
      ...
      while (scan.hasNextLine()) {
        ...
        board.setReportingDate(Date.valueOf(data[2]));
        board.setHits(Integer.parseInt(data[3]));
    ...
    try {
      ...
      for (Board board : boardList) {
        ...
        String str = String.format("%d,%s,%s,%d\n", board.getNo(), board.getContents(),
            board.getReportingDate(), board.getHits());
